...
This guide explains what an instructor needs to do to onboard new students into the PE Bootcamp lab — including how each student gets their own isolated VPC and Terraform state file, and why the OIDC + single shared IAM role approach requires the least possible setup.
...
5. [Why OIDC + a Single Shared Role Is the Right Approach](#5-why-oidc--a-single-shared-role-is-the-right-approach)
...
All of the following are **one-time setup items** that already exist in the `aws-innovationlabs-guadalajara` account. Nothing in this section needs to be repeated for each new student or cohort.
...
| IAM role | `innovation-labs-pipelines` — one role shared by all students, all cohorts |
| Template repository | This repo configured as a GitHub template so students can create their own private copy |

Because the role and the OIDC provider are already in place, **enrolling a new student requires zero AWS changes**. The instructor only needs to share the role ARN.
...
### Step 2 — Add all usernames to the role trust policy in one update

Open the `innovation-labs-pipelines` role in IAM and update the `StringLike` condition array with all student usernames at once:
...
### Step 3 — Instructor shares the role ARN with the student

The role ARN is the same for everyone:

```text
arn:aws:iam::<ACCOUNT_ID>:role/innovation-labs-pipelines
```
...
## 5. Why OIDC + a Single Shared Role Is the Right Approach
...
### The trust policy on `innovation-labs-pipelines`

The role's trust policy uses a `StringLike` condition to enumerate the GitHub usernames of enrolled students. Only workflows running under a listed username can assume the role:
...
### Why one shared role is simpler than per-student roles
...
| **One shared role (this approach)** | **Add one line to trust policy** | **Same ARN for everyone** | **Remove lines from trust policy** |

With a single shared role, enrollment is: **add one username to the trust policy, share one ARN and a CIDR**. No new roles, no new policies, no new secrets to manage in AWS.
...
- [x] IAM role `innovation-labs-pipelines` created with OIDC trust policy and lab permission policy
...
- [ ] Add all usernames to the `innovation-labs-pipelines` trust policy in one update (`"repo:<username>/*"` per student)
- [ ] Share with all students: role ARN `arn:aws:iam::<ACCOUNT_ID>:role/innovation-labs-pipelines`
...
- [ ] Remove each student's `repo:<username>/*` entry from the trust policy
